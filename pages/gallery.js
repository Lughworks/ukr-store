    ...
    return `
        <div class="min-h-screen bg-black text-white p-4 md:p-12 relative overflow-y-auto">
            <div class="flex justify-between items-center mb-16">
                <div>
                    <span class="text-[10px] text-zinc-500 font-black tracking-[0.5em] uppercase italic">Visual_Log / Evidence</span>
                    <h1 class="heading-font text-4xl md:text-7xl font-black uppercase tracking-tighter mt-2">The_Archive</h1>
                </div>
                <button onclick="window.closePage()" class="group flex items-center gap-3 bg-zinc-900 hover:bg-white hover:text-black transition-all px-8 py-4 rounded-full border border-zinc-800">
                    <span class="text-[10px] font-black uppercase tracking-widest text-inherit">Close_Archive</span>
                </button>
            </div>
                ...
                ${images.map(imgName => `
                    <div class="break-inside-avoid group relative bg-zinc-900/20 border border-zinc-800 rounded-[2rem] overflow-hidden">
                        <img src="./images/gallery/${imgName}" 
                             onerror="this.style.display='none'; this.parentElement.innerHTML += '<div class=\'p-12 text-center text-[8px] text-zinc-700 uppercase font-black tracking-widest\'>Data_Missing: ${imgName}</div>'" 
                        ...
                        <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-8 flex flex-col justify-end pointer-events-none">
                            <span class="text-[8px] text-white/50 font-mono uppercase tracking-[0.4em]">Asset_ID: ${imgName.split('.')[0]}</span>
                        </div>
                    </div>
                `).join('')}
            </div>
